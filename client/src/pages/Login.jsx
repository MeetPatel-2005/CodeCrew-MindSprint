            ...
            if (roleFromLogin === "donor") {
              ...
              try {
                const profile = await axios.get(
                  backendUrl + "/api/donor/profile"
                );
                if (
                  profile.data?.success &&
                  profile.data?.profile?.profileCompleted
                ) {
              ...
              const needsOnboarding =
                !userDataResponse.phone || !userDataResponse.bloodGroup;
              navigate(
                needsOnboarding ? "/patient-onboarding" : "/patient-dashboard"
              );
            }
          } else {
            navigate("/email-verify");
        ...
        <div className="bg-white p-10 rounded-2xl shadow-2xl w-full sm:w-96 text-gray-700 text-sm border border-green-100">
          ...
          <form onSubmit={onSubmitHandler}>
            ...
            <p
              ...
              className="mb-4 text-[#68F432] cursor-pointer hover:text-green-600 transition-colors duration-200"
            >
              Forget Password ?
            </p>
            ...
            <button
              type="submit"
              className="w-full py-2.5 rounded-full bg-gradient-to-r from-[#68F432] to-green-500 text-white font-medium hover:from-green-500 hover:to-[#68F432] transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
            >
              {state}
            </button>
          </form>
